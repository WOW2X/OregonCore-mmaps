...
void Unit::SendMonsterMoveByPath(Path<Elem, Node> const& path, uint32 start, uint32 end, uint32 traveltime)
{
    ...
    if (GetUnitMovementFlags() & SplineFlags(SPLINEFLAG_FLYING))
    {
    ...
    else
    {
        ...
        float mid_X = (GetPositionX() + path[end-1].x) * 0.2f;
        float mid_Y = (GetPositionY() + path[end-1].y) * 0.2f;
        float mid_Z = (GetPositionZ() + path[end-1].z) * 0.2f;
    ...
    switch (spellInfo->DmgClass)
    {
        ...
        case SPELL_DAMAGE_CLASS_MELEE:
        {
            ...
            if (damageSchoolMask & SPELL_SCHOOL_MASK_NORMAL)
            {
                ...
                blocked = isSpellBlocked(pCaster, spellInfo, attackType);
            }
